...
**Question 1: [A]**Make a simple time-series plot of the Harvard Forest (HARV) Phenocam phenology data (gcc_90), which EFI has preprocessed and saved here https://data.ecoforecast.org/neon4cast-targets/phenology/phenology-targets.csv.gz as part of the NEON forecasting challenge.  

```{r Question_1}
phenology = readr::read_csv("https://data.ecoforecast.org/neon4cast-targets/phenology/phenology-targets.csv.gz")
head(phenology)
pheno = phenology |> filter(site_id == "HARV", variable == "gcc_90")
plot(pheno$datetime, pheno$observation, type='l')
...
x = grep("^#",myCode)    ## returns all the lines that begin with the comment character, #.
myCode[x]
...
lines(pheno$datetime, pheno$observation, col='red')
...
**Question 4: [A]** Use arrow to access the NOAA GEFS weather forecast and filter this data to the forecast issued yesterday at midnight for the NEON site_id "HARV". The EFI NEON challenge stores this data at the endpoint "data.ecoforecast.org" in a S3 bucket named "neon4cast-drivers/noaa/gefs-v12/stage2/parquet/0/2024-01-01". Note in this case that once could select a different data by changing the last folder referenced, or could access multiple dates by leaving of a date at the end of the bucket.
 Make a timeseries plot of the "air_temperature" data in this forecast that shows one curve for each ensemble member (there are 31 ensemble members total). Hint: make sure to filter all relevant variables before calling `collect`.

```{r Question_4}
## connect to the S3 bucket
s3 <- arrow::s3_bucket(bucket = "neon4cast-drivers/noaa/gefs-v12/stage2/parquet/0/2024-01-07",
                        endpoint_override = "data.ecoforecast.org",
                        anonymous=TRUE)
dsa <- arrow::open_dataset(s3)
dsa
df <- dsa |> 
  filter(site_id == "HARV", 
...
## plot
dfa = subset(df,parameter == 1)
plot(dfa$datetime, dfa$prediction, type = 'l')

for (i in 2:31){
  dfa = subset(df,parameter == i)
  lines(dfa$datetime, dfa$prediction, col = i)
}
...
attributes_table = get_attributes(dt_md$dataTable$attributeList)
attributes_df <- do.call(rbind, lapply(attributes_table, data.frame))
att_df_subset = subset(attributes_df, attributeName == c("species_1","species_2"))
att_df_subset

```

```{r Question_5 B}
metadata_file = eml_get(md$additionalMetadata, "forecast")
metadata_file$process_error$status
```
...
1. Script with a conditional to check if the data is up to date

#### Script pseudocode --> `update_version.R`
```
if (file.exists("MODIS_Data")) {
  load("MODIS_Data")
  last_date = max(calendar_date)
  if (last_date < Sys.Date()) {
    ## Update data from MODIS only for the missing dates
  }
} else {
  ## pass
...
2. Cron job to run the script daily at 4AM
```
date_v <- system.file(package = "cronR", "extdata", "update_version.R")
cmd <- cron_rscript(date_v)
cron_add(command = cmd, frequency = 'daily', at='4AM', id = 'date_verifier')
```
...
```
MAILTO = bastian6@bu.edu
0 4 * * 0 /bastian/EF_Activities/update_version.R
```
