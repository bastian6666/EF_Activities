...
points(pheno$datetime, pheno$observation, col='red')
...
```{r Question_4}
s3_b <- arrow::s3_bucket(bucket = "neon4cast-drivers/noaa/gefs-v12/stage2/parquet/0/2024-01-01",
                        endpoint_override = "data.ecoforecast.org",
                        anonymous=TRUE)
ds_b <- arrow::open_dataset(s3_b)
df_b <- ds_b |> 
  filter(site_id == "HARV") |>
  collect()
head(df_b)
...
```{r}
#Make a timeseries plot of the "air_temperature" data in this forecast that shows one curve for each ensemble member (there are 31 ensemble members total).
unique(df_b$ensemble_member_id)

plot(df_b$reference_datetime, df_b$prediction, type='l')
```
