...
1.  Describe the trajectory observed in each case.

For r = 1.95: With this variation of the intrinsic growth rate the population size increases to the maximum value and then fluctate around the carrying capacity value across the time, slowly converging to it (not reached in the time frame of the simulation).

For r = 2.05: In ths case we observe a similar behavior but with higher fluctations around the carrying capacity value, and a slower convergence to it.

For r = 2.5: In this case the population size increases and then fluctate slow from time 5 to 10, then the fluctation increase drastically for the rest of the time frame.

For r = 2.8: For this plot we observe the normal increase in the population size, but then the fluctations drastically increases without any visible pattern or tendency to converge to the carrying capacity value.
...
abline(v=0)				          
lines(x,dgamma(x,2,1),col=2)			
abline(v=2,col=2)
lines(x,dgamma(x,1,2),col=3)
abline(v=1,col=3)
```

```{r}
# Cumulative distribution function, for Gamma distribution.
plot(x,pgamma(x,1,1),type='l')
abline(v=0)
lines(x,pgamma(x,2,1),col=2)
abline(v=2,col=2)
lines(x,pgamma(x,1,2),col=3)
abline(v=1,col=3)
```
...
p = seq(0,1,by=0.01)
plot(p,qgamma(p,1,1),type='l',ylim=range(x))    # ylim sets the y-axis range
# range returns the min/max as a 2-element vector
abline(h=0)						# “h” for “horizontal”
lines(p,qgamma(p,2,1),col=2)
abline(h=2,col=2)
lines(p,qgamma(p,1,2),col=3)
abline(h=1,col=3)
```

```{r}
qgamma(c(0.025,0.975),1,1)		   
plot(p,qgamma(p,1,1),type='l',ylim=range(x))
abline(v=c(0.025,0.975),lty=2)	
abline(h=qgamma(c(0.025,0.975),1,1),lty=2)	
plot(x,dgamma(x,1,1),type='l')		
abline(v=qgamma(c(0.025,0.975),1,1),lty=2)
```

```{r}
# Random function, for Gamma distribution.
n = c(10,100,1000,10000)	
for(i in 1:4){			
  hist(rgamma(n[i],1,1),main=n[i],probability=TRUE,breaks=40)  
...
hist(rE,main="r",breaks=40)
hist(KE,main="K",breaks=40)
...
```{r}
plot(time,n[1,],type='l',ylim=c(0,12),lwd=3,
     bty='l',cex.lab=1.5,
     xlab="Time",ylab="Population Size")
for(i in 2:10){
  lines(time,n[i,],lwd=3)
}
...
hist(n[,15],main="Population at time = 15",breaks=40)
...
```{r}
plot(time,n.stats[2,],type='l',ylim=c(0,12),lwd=3,
     bty='l',cex.lab=1.5,
     xlab="Time",ylab="Population Size")
...
plot(time,n.stats[2,],type='l',ylim=c(0,12),lwd=3,
     bty='l',cex.lab=1.5,
     xlab="Time",ylab="Population Size")
ciEnvelope(time,n.stats[2,],n.stats[3,])
```
...
for (rate in r){
  ...
  for(i in 1:NE){
    for(t in 2:NT){
      n[i,t] = max(0,n[i,t-1] + rate*n[i,t-1]*(1-n[i,t-1]/K))
    }
  }
  n.stats = apply(n,2,quantile,c(0.025,0.5,0.975))
  plot(time,n.stats[2,],type='l',ylim=c(0,12),lwd=3,
