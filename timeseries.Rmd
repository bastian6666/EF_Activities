...
Ar \_\_\> Autoregressive --\> P=0

$$\rho_1x_t + \rho_2x_{t-1} + \rho_3x_{t-2}$$

I --\> Integrative --\> D = 0 $$x_t <-- [x_t - x_{t-1}] --> \approx \frac{dx}{dt}$$ MA --\> Moving Average --\> Q=0 $$C(P,D,Q)$$

```{r}
library(forecast)
rw = Arima(xt,c(0,0,0)) # Autoregressive 
rw
```
