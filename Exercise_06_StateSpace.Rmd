...
# Create a plot
library(ggplot2)
ggplot(ci_data, aes(x = Model, ymin = Lower, ymax = Upper, y = (Lower + Upper) / 2)) +
  geom_errorbar(width = 0.2) +
  geom_point() +
  ylab("tau_obs") +
  ggtitle("95% Confidence Intervals for tau_obs")
...
* Explain and discuss the parameter estimates (betas) from the dynamic linear model (what do they mean both biologically and in terms of the predictability of the system) and their correlations
  ...
  - The summary of the dynamic linear model (DLM) tells us how different factors, like baseline conditions (betaIntercept), minimum temperature (betaTmin), and another factor (betaX), affect what we're studying. The baseline condition shows what happens when other factors are not at play. The temperature part indicates that as the temperature goes up, the thing we're looking at tends to go down a bit, suggesting that cooler temperatures might be better up to a point. The other factor also leads to decreases, implying it's important to consider for predicting outcomes. The model helps us understand how these factors work together and their impact, which is crucial for making accurate predictions, especially in situations where things change over time.
...
ecoforecastR::ciEnvelope(time,ci[1,],ci[3,],col=ecoforecastR::col.alpha("lightBlue",0.75))
points(time,y,pch="+",cex=0.5)
ecoforecastR::ciEnvelope(time,ci2[1,],ci2[3,],col=ecoforecastR::col.alpha("lightRed",0.75))
